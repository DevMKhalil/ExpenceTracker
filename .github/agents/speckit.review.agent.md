---
description: Review implementation code against spec.md, plan.md, tasks.md, and constitution.md. Writes detailed findings to review.md in the feature directory for the fix agent to consume.
...
- **Code**:
  ...
  {
      _db.Execute($"INSERT INTO payments VALUES ({amount})");  // SQL injection
  }
  ```
- **Problem**: Direct string interpolation in SQL query allows SQL injection attacks. This violates FR-007 (secure payment processing) and OWASP A03 (Injection).
- **Fix**:
  1. Replace the raw SQL with a parameterized query
  2. Use `@amount` parameter placeholder
  3. Pass `amount` as a SqlParameter object
  4. Expected result:
     ```csharp
     public void ProcessPayment(decimal amount)
     {
         _db.Execute("INSERT INTO payments VALUES (@amount)", new { amount });
     }
     ```
...
|-------------|-------|--------|
| R1.C1 | SQL injection in PaymentService | ✅ Resolved |
| R1.H3 | Missing null check in ExpenseRepository | ✅ Resolved |
...
After writing the file, output a concise summary (do NOT repeat the full report in chat):
